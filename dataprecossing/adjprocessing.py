...
datasets = 'DJIA'
data_addr = 'C:/Users/Gursimranjeet Singh/REGCN/data/data/'
data = np.load(data_addr+datasets+'.npy',allow_pickle=True)
threshold = 0.9
for j in range(data.shape[0]):
    adj = []
    file = glob.glob(os.path.join("C:/Users/Gursimranjeet Singh/REGCN/data/data/VMDdata/%s/%s*.csv" % (datasets, j)))
    for f in file:
    ...
    np.save('C:/Users/Gursimranjeet Singh/REGCN/data/adj/' + datasets + '/' + datasets + '_VMD_'+str(j)+'.npy', adj)
    adj = np.array(adj)
    adj[adj < threshold] = 0
    np.save('C:/Users/Gursimranjeet Singh/REGCN/data/adj/' + datasets + '/' + datasets + '_VMD_' + str(j) + '_90.npy', adj)
